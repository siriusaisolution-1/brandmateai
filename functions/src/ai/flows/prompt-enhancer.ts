...
export const enhancePromptFlow = ai.defineFlow({
  ...
  inputSchema: z.object({ userId: z.string(), brandId: z.string(), basePrompt: z.string(), taskType: z.enum(['image','video','blog','social']), style: z.string().optional() }),
  outputSchema: z.object({ enhancedPrompt: z.string() })
}, async (input) => {
  const userPrompt = `Base Prompt: \"${input.basePrompt}\"\\nTask Type: ${input.taskType}\\nUser ID: ${input.userId}\\nDesired Style: ${input.style ?? 'Default'}`;
